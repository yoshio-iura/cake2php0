...
<h2><?php echo __d('cake', 'View %s', __($singularHumanName)); ?></h2>
	<dl>
<?php
...
<?php
	echo "\t\t<li>";
	echo $this->Html->link(__d('cake', 'Edit %s', __($singularHumanName)), array('action' => 'edit', ${$singularVar}[$modelClass][$primaryKey]));
	echo " </li>\n";

	echo "\t\t<li>";
	echo $this->Form->postLink(__d('cake', 'Delete %s', __($singularHumanName)), array('action' => 'delete', ${$singularVar}[$modelClass][$primaryKey]), array(), __d('cake', 'Are you sure you want to delete # %s?', ${$singularVar}[$modelClass][$primaryKey]));
	echo " </li>\n";

	echo "\t\t<li>";
	echo $this->Html->link(__d('cake', 'List %s', __($pluralHumanName)), array('action' => 'index'));
	echo " </li>\n";

	echo "\t\t<li>";
	echo $this->Html->link(__d('cake', 'New %s', __($singularHumanName)), array('action' => 'add'));
	echo " </li>\n";
	...
	foreach ($associations as $_type => $_data) {
		foreach ($_data as $_alias => $_details) {
			if ($_details['controller'] != $this->name && !in_array($_details['controller'], $done)) {
				echo "\t\t<li>";
				echo $this->Html->link(
					__d('cake', 'List %s', __(Inflector::humanize($_details['controller']))),
					array('plugin' => $_details['plugin'], 'controller' => $_details['controller'], 'action' => 'index')
				);
				echo "</li>\n";
				echo "\t\t<li>";
				echo $this->Html->link(
					__d('cake', 'New %s', __(Inflector::humanize(Inflector::underscore($_alias)))),
...
<div class="related">
	<h3><?php echo __d('cake', "Related %s", __(Inflector::humanize($_details['controller']))); ?></h3>
...
<?php endif; ?>
	<div class="actions">
		...
		<li><?php
			echo $this->Html->link(
				__d('cake', 'Edit %s', __(Inflector::humanize(Inflector::underscore($_alias)))),
...
?>
		<th class="actions"><?php __d('cake', 'Actions');?></th>
	</tr>
<?php
		$i = 0;
